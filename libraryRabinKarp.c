...
bool rabinKarp(const char *pat, const char *txt) {
...
struct Book *searchBookHashTable(const struct HashTable *hashTable, const char *key, const char *searchType) {
    ...
    while (current != NULL) {
        if ((strcmp(searchType, "ISBN") == 0 && rabinKarp(key, current->key)) ||
            (strcmp(searchType, "Author") == 0 && rabinKarp(key, current->value.author)) ||
            (strcmp(searchType, "Title") == 0 && rabinKarp(key, current->value.title))) {
...
void searchBooksHash(struct HashTable *hashTable, const char *searchType, const char *searchTerm) {
    ...
    for (int i = 0; i < TABLE_SIZE; i++) {
        ...
        while (current != NULL) {
            if ((strcmp(searchType, "ISBN") == 0 && rabinKarp(searchTerm, current->key)) ||
                (strcmp(searchType, "Author") == 0 && rabinKarp(searchTerm, current->value.author)) ||
                (strcmp(searchType, "Title") == 0 && rabinKarp(searchTerm, current->value.title))) {
                printf("%-4d", ++count);
